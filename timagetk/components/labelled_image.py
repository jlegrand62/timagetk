...
# -- Define accepted str for 'label' parameter:
LABEL_STR = ['all', 'L1', 'L2']
...
def image_with_labels(image, labels, boundingbox, erase_value=0):
    ...
    return SpatialImage(template_im, voxelsize=image.get_voxelsize(),
...
class LabelledImage(SpatialImage):
    ...
    def __new__(cls, image, background=None, no_label_id=None):
        """
        """
        if isinstance(image, SpatialImage):
            ...
            msg = "Unknown type of image, can be a SpatialImage or a numpy.ndarray!"
            raise TypeError(msg)

        return SpatialImage.__new__(cls, input_array, origin=origin,
                                    voxelsize=voxelsize, dtype=dtype,
                                    metadata_dict=metadata_dict)

    def __init__(self, image, background=None, no_label_id=None):
        """
        Parameters
        ----------
        image: SpatialImage
    ...
    def fuse_labels_in_image(self, labels, value='min', verbose=True):
        ...
        Returns
        -------
        Nothing, modify the SpatialImage in 'self.image'
        """
        ...
        relab_img = SpatialImage(array, origin=self.get_origin(),
                                 voxelsize=self.get_voxelsize(),
                                 metadata_dict=self.get_metadata())
        # - Re-initialisation of the object with the relabelled LabelledImage:
        self.__new__(relab_img, background=self.background,
    ...
    def remove_labels_from_image(self, labels, verbose=True):
        ...
        Returns
        -------
        Nothing, this function modify the SpatialImage 'self.image' !
        """
        if isinstance(labels, int):
            labels = [labels]
        has_back_id = False
        ...
        # self._re_init_array(array, self.get_origin(), self.get_voxelsize(),
        #                     self.get_metadata(), self.background,
        #                     self.no_label_id)
    ...
    def relabel_from_mapping(self, mapping, clear_unmapped=False, **kwargs):
        ...
        **kwargs
        ...
        # - Re-initialise the object:
        # self._re_init_array(relab_img, self.get_origin(), self.get_voxelsize(),
        #                     self.get_metadata(), self.background,
        #                     self.no_label_id)
        # - May print about elapsed time:
        if verbose:
    ...
    # @classmethod
    # def _re_init_array(cls, array, ori, vxs, md, back_id, no_id):
    #     """
    ...
    #     """
    #     # - Re-initialisation of the object with the relabelled LabelledImage:
    #     super(LabelledImage, cls).__new__(cls, array, origin=ori, voxelsize=vxs,
    #                                       metadata_dict=md)
    #     super(LabelledImage, cls).__init__(cls, array, background=back_id,
    #                                        no_label_id=no_id)
    #
    #     return

    def _re_init_array(self, array, ori, vxs, md, back_id, no_id):
        """
        Hidden function rebuilding the object when labelled image is modified.
        ...
        """
        # - Make a SpatialImage out of the array:
        # array = SpatialImage(array, origin=self.get_origin(),
        #                          voxelsize=self.get_voxelsize(),
        #                          metadata_dict=self.get_metadata())
        array = SpatialImage(array, origin=ori, voxelsize=vxs, metadata_dict=md)
        # - Re-initialisation of the object with the relabelled LabelledImage:
        self.__init__(array, background=back_id, no_label_id=no_id)
