...
def imgtosets(sp_img, label=[], background_id=None):
    ...
    background_id: int, optional
        background_id, removed from the computation if specified
    ...
    if label is None or label == []:
        ...
        check_lab = [0 if lab in lab_max else 1 for ind, lab in
                     enumerate(label)]
    ...
    for ind in bounding_box_dict:
        bbox, lab, glob_set = bounding_box_dict[ind]["Bounding box"], \
                              bounding_box_dict[ind]["Label"], []
        range_x, range_y = xrange(bbox[0], bbox[1] + 1), xrange(bbox[2],
                                                                bbox[3] + 1)
