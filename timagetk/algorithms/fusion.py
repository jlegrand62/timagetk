...
def fusion(list_images, iterations=None, man_trsf_list=None,
    ...
    if man_trsf_list is None:
        ...
        conds_init = conds_init and isinstance(list_images, list) and len(
            list_images) - 1 == len(man_trsf_list)
    ...
    if conds_init and 1 not in conds_list_img and 1 not in conds_list_trsf:
        ...
        init_ref = apply_trsf(list_images[0], trsf=None,
        ...
        for ind, sp_img in enumerate(list_images):
            if ind > 0:
                ...
                print "...Rigid registration{}...".format(
                    " with 'init-res-trsf'" if man_trsf_list[
                                                   ind - 1] is not None else "")
                trsf_rig, res_rig = blockmatching(sp_img, init_ref,
                                                  init_result_transformation=
                                                  man_trsf_list[ind - 1],
                                                  param_str_2='-trsf-type rigid -py-ll 1{}'.format(
                                                      " -time" if time else ""))
                ...
                trsf_aff, res_aff = blockmatching(sp_img, init_ref,
                                                  ...
                                                  param_str_2='-trsf-type affine{}'.format(
                                                      " -time" if time else ""))
                ...
                trsf_def, res_def = blockmatching(sp_img, init_ref,
                                                  ...
                                                  param_str_2='-trsf-type vectorfield {}{}'.format(
                                                      vf_params,
                                                      " -time" if time else ""))
        ...
        for index in range(0, iterations):
            ...
            if mean_imgs_prefix != "":
                print "Saving the mean image..."
                imsave(
                    mean_imgs_prefix + "-mean_img_iter{}.inr".format(index + 1),
                    mean_ref_update)
            del mean_ref, mean_trsf_inv
        ...
        print "Specifications log: conds_init={}; conds_list_img={}; conds_list_trsf={}".format(
            conds_init, conds_list_img, conds_list_trsf)
        return
