...
class TemporalMatching(object):
    ...
    def __init__(self, segmentation_list, feature_space_list=None,
        ...
        segmentation_list: list
            list of input ``SpatialImage`` (segmentations)
        feature_space_list: list
            optional, list of input feature_space dictionaries
        background_id_list: list
            optional, list of input background identifiers
        """
        ...
        seg_arr_list = [
            np_array(segmentation_list[ind], dtype=segmentation_list[ind].dtype)
            for ind, val in
            enumerate(segmentation_list)]  # --- numpy array instance
        self.segmentation_list = seg_arr_list
        self._voxelsize_list = [segmentation_list[ind].voxelsize for ind, val in
                                enumerate(segmentation_list)]
        ...
        if background_id_list:
            try:
                ...
                raise TypeError(
                    "Input 'background_id_list' must be a list of background label")
            try:
                assert len(background_id_list) == len(segmentation_list)
            except AssertionError:
                raise ValueError(
                    "Input 'background_id_list' must be of the same length than 'segmentation_list'")
        ...
        if feature_space_list:
            ...
            try:
                ...
                raise ValueError(
                    "Input 'feature_space_list' must be of the same length than 'segmentation_list'")
            ...
            try:
                ...
                raise TypeError(
                    "Input 'feature_space_list' must be a list of dictionaries")
    ...
    def get_segmentation_list(self):
        ...
        """
        sp_img_list = [SpatialImage(self.segmentation_list[ind],
                                    voxelsize=self._voxelsize_list[ind])
    ...
    def set_feature_space_list(self, feature_space_list):
        ...
        """
        conds_list = isinstance(feature_space_list, list) and len(
            feature_space_list) == len(self.segmentation_list)
        ...
        if conds_list and 1 not in conds_feat:
            for ind, feat_space_dict in enumerate(feature_space_list):
                for key in feat_space_dict:
                    if feat_space_dict[key]['Label'] == self.back_id_list[ind]:
                        del feat_space_dict[
                            key]  # if present remove background features
                feature_space_list[ind] = feat_space_dict
            self.feature_space_list = feature_space_list
    ...
    def set_admissible_matching_list(self, adm_match_list):
        ...
        adm_match_list: list
            list of dictionaries
        """
        conds_list = isinstance(adm_match_list, list) and len(
            adm_match_list) == len(self.segmentation_list) - 1
        if conds_list:
    ...
    def set_single_cost_list(self, sing_cost_list):
        ...
        sing_cost_list: list
            list of dictionaries
        """
        conds_list = isinstance(sing_cost_list, list) and len(
            sing_cost_list) == len(self.segmentation_list) - 1
        if conds_list:
    ...
    def set_pairwise_cost_list(self, pairw_cost_list):
        ...
        Parameters
        ----------
        pairw_cost_list: list
            list of dictionaries
        """
        conds_list = isinstance(pairw_cost_list, list) and len(
            pairw_cost_list) == len(self.segmentation_list) - 1
        if conds_list:
            self.pairwise_cost_list = pairw_cost_list

    def get_normalized_cost_list(self, criterion=None, min_cost=None,
                                 max_cost=None):
        """
    ...
    def set_normalized_cost_list(self, norm_cost_list):
        ...
        norm_cost_list: list
            list of dictionaries
        """
        conds_list = isinstance(norm_cost_list, list) and len(
            norm_cost_list) == len(self.segmentation_list) - 1
        if conds_list:
    ...
    def labels_to_nodes(feat_space_dict, min_node_val=1):
        ...
        -------
        nodes_dict: dict
            dictionary of contiguous vertices
        ...
        nodes_dict = {}
        nodes_list = chain(
            xrange(min_node_val, min_node_val + len(feat_space_dict)))
    ...
    def compute_nodes_list(self):
        ...
        for ind, feat_space_dict in enumerate(feature_space_list):
            if ind == 0:
                nodes_list.append(
                    self.labels_to_nodes(feature_space_list[ind], 1))
            elif ind > 0:
                nodes_list.append(self.labels_to_nodes(feature_space_list[ind],
                                                       max(nodes_list[
                                                               ind - 1].keys()) + 1))
        return nodes_list
    ...
    def compute_admissible_matching(self, inc_max=None):
        ...
        ----------
        inc_max: int
            maximal increment (neighborhood information), default None
        ...
        for glob_index, feat_dict in enumerate(feature_space_list):
            if glob_index < len(feature_space_list) - 1:
                ...
                if inc_max is None:
                    ...
                    bbox_size_list = [feat_0[key]['Bounding box size'] for key
                                      in feat_0]
                    x_list = [bbox_size_list[ind][0] for ind, val in
                ...
                nodes_0, nodes_1 = nodes_list[glob_index], nodes_list[
                    glob_index + 1]
                matrix_1, shape_1, back_id_1 = segmentation_list[
                                                   glob_index + 1], \
                                               segmentation_list[
                                                   glob_index + 1].shape, \
                                               self.back_id_list[glob_index + 1]

                for key in nodes_0:
                    adm_match_dict[key] = {}
                    adm_match_dict[key]['Initial node'], adm_match_dict[key][
                        'Initial label'] = [key], [nodes_0[key]['Label']]
                    lab = []

                    index_x, index_y = nodes_0[key]['Index centroid'][0], \
                                       nodes_0[key]['Index centroid'][1]
                    range_x, range_y = xrange(0, shape_1[0]), xrange(0,
                                                                     shape_1[1])
                    if (img_dim == 3):
                        index_z, range_z = nodes_0[key]['Index centroid'][
                                               2], xrange(0, shape_1[2])

                    if (img_dim == 2) and (index_x in range_x) and (
                            index_y in range_y):
                        ...
                        if (init_label == back_id_1):
                            ...
                            if ((shape_1[0] - 1 - index_x) >= index_x):
                                while (init_label == back_id_1):
                                    step_x += 1
                                    if ((index_x + step_x < shape_1[0]) and (
                                            step_x < inc_max)):
                                        init_label = matrix_1[
                                            index_x + step_x, index_y]
                                        if ((init_label != back_id_1) and (
                                                len(lab) == 0)):
                                ...
                                while (init_label == back_id_1):
                                    step_x -= 1
                                    if (index_x + step_x >= 0) and (
                                            np_abs(step_x) < inc_max):
                                        init_label = matrix_1[
                                            index_x + step_x, index_y]
                                        if ((init_label != back_id_1) and (
                                                len(lab) == 0)):
                    ...
                    elif (img_dim == 3) and (index_y in range_x) and (
                            index_y in range_y) and (index_z in range_z):
                            ...
                            if ((shape_1[0] - 1 - index_x) >= index_x):
                                while (init_label == back_id_1):
                                    ...
                                    if ((index_x + step_x < shape_1[0]) and (
                                            step_x < inc_max)):
                                        init_label = matrix_1[
                                            index_x + step_x, index_y, index_z]
                                        if ((init_label != back_id_1) and (
                                                len(lab) == 0)):
                                            lab.append(init_label)
                                    else:
                ...
                rem_keys = [key for key in adm_match_dict
                            if
                            not adm_match_dict[key].has_key('Admissible nodes')]
                if rem_keys != []:
                    for val in rem_keys:
    ...
    def compute_single_cost(self, adm_match_list, criterion=None):
        ...
        for glob_index, nodes_dict in enumerate(nodes_list):
            if glob_index < len(nodes_list) - 1:
                segmentation_0, segmentation_1 = segmentation_list[glob_index], \
                                                 segmentation_list[
                                                     glob_index + 1]
                nodes_0, nodes_1 = nodes_list[glob_index], nodes_list[
                    glob_index + 1]
                ...
                for key in adm_match_list[glob_index]:
                    ...
                    single_cost_dict[key]['Initial node'] = \
                        adm_match_list[glob_index][key]['Initial node']
                    single_cost_dict[key]['Criterion'] = criterion
                    single_cost_dict[key]['Admissible nodes'] = \
                        adm_match_list[glob_index][key]['Admissible nodes']
                    ...
                    if (criterion == 'Euclidean distance'):
                        for node in single_cost_dict[key]['Admissible nodes']:
                            if (segmentation_0.get_dim() == 2):
                                eucl_dist = np_sqrt(pow((nodes_0[key][
                                                             'Physical centroid'][
                                                             0] - nodes_1[node][
                                                             'Physical centroid'][
                                                             0]), 2) +
                                ...
                                eucl_dist = np_sqrt(pow((nodes_0[key][
                                                    ...
                                                    pow((nodes_0[key][
                                                             ...
                                                             2]), 2))
                            single_cost_dict[key]['Association cost'][
                                node] = around_val(eucl_dist)

                    elif ((criterion == 'Jaccard coefficient') or (
                            criterion == 'Mean overlap')
                          or (criterion == 'Target overlap') or (
                                  criterion == 'Volume similarity')
                          or (criterion == 'False negative error') or (
                                  criterion == 'False positive error')
                          or (criterion == 'Sensitivity') or (
                                  criterion == 'Conformity')):
                        set_1 = compute_res_sets(segmentation_0, [key], nodes_0)
                        for node in single_cost_dict[key]['Admissible nodes']:
                            set_2 = compute_res_sets(segmentation_1, [node],
                                                     nodes_1)
                            metric = sets_metrics(set_1, set_2, criterion)
                            single_cost_dict[key]['Association cost'][
                                node] = around_val(metric)
    ...
    def compute_pairwise_cost(self, adm_match_list, criterion=None):
        ...
        Returns
        -------
        pairwise_cost_list: list
            list of pairwise association costs
        ...
        for glob_index, node_dict in enumerate(nodes_list):
            if glob_index < len(nodes_list) - 1:
                ...
                for key in adm_match_list[glob_index]:
                    ...
                    for n1 in adm_match_list[glob_index][key][
                        'Admissible nodes']:
                        for n2 in adm_match_list[glob_index][key][
                            'Admissible nodes']:
                            if n1 != n2:
                                if segmentation_0.get_dim() == 2:
                                    dist = np_sqrt(pow((nodes_1[n1][
                                                            'Physical centroid'][
                                                            0] - nodes_1[n2][
                                                            'Physical centroid'][
                                                            0]), 2)
                                ...
                                poss_pairwise_dict[key][lab]['Node 1'], \
                                poss_pairwise_dict[key][lab]['Node 2'] = n1, n2
                                poss_pairwise_dict[key][lab][
                                    'Distance'] = around_val(dist)
                ...
                for key_1 in poss_pairwise_dict:
                    ...
                    for key_2 in poss_pairwise_dict[key_1]:
                        if poss_pairwise_dict[key_1][key_2][
                            'Distance'] < min_dist:
                            min_dist = poss_pairwise_dict[key_1][key_2][
                                'Distance']
                    for key_2 in poss_pairwise_dict[key_1]:
                        if poss_pairwise_dict[key_1][key_2][
                            'Distance'] == min_dist:
                            pairwise_cost_dict[key_1] = {}
                            pairwise_cost_dict[key_1]['Node 1'], \
                            pairwise_cost_dict[key_1]['Node 2'] = \
                                poss_pairwise_dict[key_1][key_2]['Node 1'], \
                                poss_pairwise_dict[key_1][key_2]['Node 2']
                ...
                if glob_index == 0:
                    ...
                    for key in pairwise_cost_list[glob_index - 1]:
                        add_node = pairwise_cost_list[glob_index - 1][key][
                            'Admissible nodes'][0]
                ...
                for key in pairwise_cost_dict:
                    ...
                    pairwise_cost_dict[key]['Initial label'] = nodes_0[key][
                        'Label']
                    pairwise_cost_dict[key]['Admissible nodes'] = [mini_node]
                    n1, n2 = pairwise_cost_dict[key]['Node 1'], \
                             pairwise_cost_dict[key]['Node 2']
                    pairwise_cost_dict[key]['Final label 1'], \
                    pairwise_cost_dict[key]['Final label 2'] = nodes_1[n1][
                                                                   'Label'], \
                                                               nodes_1[n2][
                                                                   'Label']
                    ...
                    if (criterion == 'Euclidean distance'):
                        if segmentation_0.get_dim() == 2:
                            pairwise_centroid = [(nodes_1[n1][
                                                 ...
                                                 (nodes_1[n1][
                                                  ...
                                                  nodes_1[n2][
                                                      'Physical centroid'][
                                                      1]) / 2.0]
                            dist = np_sqrt(pow((nodes_0[key][
                                                    'Physical centroid'][0] -
                                                pairwise_centroid[0]), 2)
                                           + pow((nodes_0[key][
                                                      'Physical centroid'][1] -
                                                  pairwise_centroid[1]), 2))
                            ...
                            dist = np_sqrt(pow((nodes_0[key][
                                           ...
                                           + pow((nodes_0[key][
                                                      'Physical centroid'][2] -
                                                  pairwise_centroid[2]), 2))

                        pairwise_cost_dict[key]['Association cost'][
                            mini_node] = around_val(dist)
                        ...
                        set_1 = compute_res_sets(segmentation_0, [key], nodes_0)
                        set_2 = compute_res_sets(segmentation_1, [n1, n2],
                                                 nodes_1)
                        metric = sets_metrics(set_1, set_2, criterion)
                        pairwise_cost_dict[key]['Association cost'][
                            mini_node] = around_val(metric)

                    del pairwise_cost_dict[key]['Distance']
                    mini_node = mini_node + 1

                init_keys = [key for key in pairwise_cost_dict]
                add_keys = [pairwise_cost_dict[key]['Admissible nodes'][0] for
                            key in init_keys]
    ...
    def compute_normalized_cost(self, adm_match_list, criterion=None,
        ...
        max_cost: int
            optional, maximum cost (default max_cost = 100)
        ...
        pairwise_cost_list = self.compute_pairwise_cost(adm_match_list,
                                                        criterion)
        ...
        for glob_index, single_cost_dict in enumerate(single_cost_list):
            global_cost_dict, pairwise_cost_dict = single_cost_dict.copy(), \
                                                   pairwise_cost_list[
                                                       glob_index]

            init_add_list = [(pairwise_cost_dict[key]['Initial node'][0], key)
                             for key in pairwise_cost_dict]
            for tmp in init_add_list:
                additional_node = tmp[1]
                additional_cost = \
                    pairwise_cost_dict[additional_node]['Association cost'][
                        additional_node]
                global_cost_dict[tmp[0]]['Admissible nodes'].append(
                    additional_node)
                global_cost_dict[tmp[0]]['Association cost'][
                    additional_node] = additional_cost
        ...
        for global_idx, global_cost_dict in enumerate(global_cost_list):
            criterion = global_cost_dict[global_cost_dict.keys()[0]][
                'Criterion']
            if ((criterion == 'Jaccard coefficient') or (
                    criterion == 'Mean overlap')
                    or (criterion == 'Sensitivity') or (
                            criterion == 'Target overlap')):
                ...
                for key_1 in global_cost_dict:
                    for key_2 in global_cost_dict[key_1][
                        'Association cost'].keys():
                        val = 1.0 - global_cost_dict[key_1]['Association cost'][
                            key_2]
            ...
            for key_1 in global_cost_dict:
                ...
                normalized_cost_dict[key_1]['Initial node'] = \
                    global_cost_dict[key_1]['Initial node']
                normalized_cost_dict[key_1]['Admissible nodes'] = \
                    global_cost_dict[key_1]['Admissible nodes']
                for key_2 in global_cost_dict[key_1]['Association cost'].keys():
                    if global_cost_dict[key_1]['Association cost'][
                        key_2] >= val_max:
                        val_max = global_cost_dict[key_1]['Association cost'][
                            key_2]
                    if global_cost_dict[key_1]['Association cost'][
                        key_2] < val_min:
                        val_min = global_cost_dict[key_1]['Association cost'][
                            key_2]

            for key_1 in global_cost_dict:
                normalized_cost_dict[key_1]['Association cost'] = {}
                normalized_cost_dict[key_1]['Criterion'] = criterion
                normalized_cost_dict[key_1]['Min cost'], \
                normalized_cost_dict[key_1]['Max cost'] = min_cost, max_cost
                for key_2 in global_cost_dict[key_1]['Association cost'].keys():
                    normalized_val = (max_cost - min_cost) * ((global_cost_dict[
                                                                   key_1][
                                                                   'Association cost'][
                                                                   key_2] - val_min) / (
                                                                      val_max - val_min)) + min_cost
                    normalized_cost_dict[key_1]['Association cost'][
                        key_2] = int(normalized_val)
    ...
    def build_init_graph(self, normalized_cost_list, alpha_cost=None):
        ...
        ----------
        normalized_cost_list: list
            list of normalized costs
        alpha_cost: int
            optional, initial cost (default alpha_cost = int((5/100.0)*max_cost))
        ...
        sets_single = [nodes_dict.keys() for glob_idx, nodes_dict in
                       enumerate(nodes_list)]
        sets_pairwise = [pairwise_cost_dict.keys()
                         for glob_idx, pairwise_cost_dict in
                         enumerate(pairwise_cost_list)]

        self.appear, self.disappear = np_max(sets_pairwise[-1]) + 1, np_max(
            sets_pairwise[-1]) + 2
        self.source, self.sink = np_min(sets_single[0]) - 1, np_max(
            sets_pairwise[-1]) + 3
        ...
        edge_list = [(self.source, i, {'capacity': 1, 'weight': 0}) for i in
                     sets_single[0]]
        G.add_edges_from(edge_list)

        # Add edges from the last set of nodes to the sink node
        edge_list = [(i, self.sink, {'capacity': 1, 'weight': 0}) for i in
                     sets_single[-1]]
        G.add_edges_from(edge_list)
        ...
        for ind, norm_cost_dict in enumerate(normalized_cost_list):
            ...
            for key_1 in norm_cost_dict:
                for key_2 in norm_cost_dict[key_1]['Association cost'].keys():
                    if key_1 in nodes_list[ind] and key_2 in nodes_list[
                        ind + 1]:
                        var = (key_1, key_2, {'capacity': 1, 'weight':
                            norm_cost_dict[key_1]['Association cost'][key_2]})
        ...
        for ind, pairwise_cost_dict in enumerate(pairwise_cost_list):
            ...
            for key in pairwise_cost_dict:
                ...
                node_1, node_2 = pairwise_cost_dict[key]['Node 1'], \
                                 pairwise_cost_dict[key]['Node 2']
                alpha = int(np_ceil((normalized_cost_list[ind][init_node][
                    'Association cost'][key]) / 2.0))
        ...
        for ind, nodes_dict in enumerate(nodes_list):
            if ind > 0:
                ...
                edge_list = [
                    (self.appear, i, {'capacity': 1, 'weight': alpha_cost}) for
                    i in nodes_dict]
                G.add_edges_from(edge_list)
        ...
        for ind, nodes_dict in enumerate(nodes_list):
            if ind < len(nodes_list) - 1:
                ...
                edge_list = [
                    (i, self.disappear, {'capacity': 1, 'weight': alpha_cost})
                    for i in nodes_dict]
                G.add_edges_from(edge_list)

        cap_appear = cap_appear_sing + cap_appear_pairw
        G.add_edge(self.source, self.appear, capacity=cap_appear, weight=0)
        G.add_edge(self.appear, self.disappear, capacity=cap_appear,
                   weight=alpha_cost)
        G.add_edge(self.disappear, self.sink,
                   capacity=(cap_appear + cap_disapp), weight=0)
    ...
    def update_graph(self, graph, matching_dict, norm_cost_list, alpha_cost):
        ...
        graph: *nx.DiGraph*
            directed graph
        matching_dict: dict
            dictionary of matchings
        normalized_association_cost_dict: dict
            dictionary of normalized costs
        pairwise_dict: dict
            dictionary of one cell - two cells events
        alpha_cost: int
            appear/disappear value
        ...
        nodes_list, pairwise_cost_list = self.get_nodes_list(), self.get_pairwise_cost_list()
        nodes = [sorted(node_dict.keys()) for ind, node_dict in
                 enumerate(nodes_list)]
        pairw_nodes = [sorted(pairwise_cost_dict.keys())
                       for ind, pairwise_cost_dict in
                       enumerate(pairwise_cost_list)]
        pairw_set = []
        ...
        for key in matching_dict:
            ...
            if matching_dict[key]['Event'] == 'Stay':
                node_set_1, node_set_2 = matching_dict[key]['Node set 1'], \
                                         matching_dict[key]['Node set 2']
                neighbors_set_1 = graph.neighbors(node_set_1)
                for neigh in neighbors_set_1:
                    if (neigh == node_set_2):
                        graph.edge[node_set_1][neigh]['weight'] = 1
                        norm_cost_list[ind_time][key]['Association cost'][
                            neigh] = 1
    ...
    def matching(self, norm_cost_list, alpha_cost):
        ...
        source, sink, appear, disappear = self.get_part_nodes()
        print('Max flow...')
        max_flow_value, max_flow_dict = nx.maximum_flow(graph, source, sink,
                                                        capacity='capacity')
        ...
        print('Min cost...')
        min_flow_cost, min_flow_dict = nx.network_simplex(H, demand='demand',
                                                          capacity='capacity',
                                                          weight='weight')
    ...
    def process_flow_dict(self, min_flow_dict):
        ...
        nodes = [sorted(node_dict.keys()) for ind, node_dict in
                 enumerate(nodes_list)]
        pairw_nodes = [sorted(pairwise_cost_dict.keys()) for
                       ind, pairwise_cost_dict in enumerate(pairwise_cost_list)]
        ...
        for key in out_dict:
            for loc_key in min_flow_dict[key]:
                if (
                        loc_key != source and loc_key != disappear and loc_key != appear
                    ...
                    for ind, nd_list in enumerate(nodes):
                        if loc_key in nd_list:
                            ...
                            out_dict[key]['Label set 1'] = \
                                nodes_list[ind - 1][key]['Label']
                            out_dict[key]['Initial label set 2'] = \
                                nodes_list[ind][loc_key]['Label']
                            out_dict[key]['Final label set 2'] = \
                                nodes_list[ind - 1][key]['Label']
                            out_dict[key]['Association cost'] = \
                                norm_cost_list[ind - 1][key][
                                    'Association cost'][
                                    loc_key]
                            stay_numb += 1
                    ...
                    for ind, pr_list in enumerate(pairw_nodes):
                        if loc_key in pr_list:
                            ...
                            out_dict[key]['Node set 1'] = key
                            out_dict[key]['Label set 1'] = nodes_list[ind][key][
                                'Label']
                            ...
                            out_dict[key]['Initial label 1 set 2'] = \
                                nodes_list[ind + 1][node_1]['Label']
                            out_dict[key]['Initial label 2 set 2'] = \
                                nodes_list[ind + 1][node_2]['Label']
                            out_dict[key]['Association cost'] = \
                                norm_cost_list[ind][key]['Association cost'][
                                    loc_key]
                            split_numb += 1

                elif (loc_key == disappear and min_flow_dict[key][
                    loc_key] != 0):
    ...
    def iterative_matching(self, norm_cost_list, graph, tolerance=None):
        ...
        tolerance: float
            optional, percentage of unmatched cells (default 10.0/100)
        ...
        nodes = [sorted(node_dict.keys()) for ind, node_dict in
                 enumerate(nodes_list)]
        number = [len(nodes[ind]) for ind, val in enumerate(nodes) if
                  ind < len(nodes) - 1]
        tol = sum(number)
        pairw_nodes = [sorted(pairwise_cost_dict.keys()) for
                       ind, pairwise_cost_dict in enumerate(pairwise_cost_list)]
        ...
        while ((tot_numb_list[-1] < (1.0 - tolerance) * tol) or (
                alpha_cost < max_cost)):
            H.add_node(source, demand=-max_flow_value)
            H.add_node(sink, demand=max_flow_value)
            print('Min cost...')
            min_flow_cost, min_flow_dict = nx.network_simplex(H,
                                                              demand='demand',
                                                              capacity='capacity',
                                                              weight='weight')
            print('Min flow value'), min_flow_cost
            ...
            H, norm_cost_list = self.update_graph(H, matching_dict,
                                                  norm_cost_list, alpha_cost)
            if ((tot_numb_list[-1] > (1.0 - tolerance) * tol) or (
                    alpha_cost == max_cost)):
    ...
    def res_images(self, matching_dict):
        ...
        img_list = []
        for glob_ind, glob_val in enumerate(
                nd_idx):  # glob_ind ---> time, # glob_val ---> set of nodes for this time
            if glob_ind < len(segmentation_list) - 1:
                init_img = self.back_id_list[glob_ind] * np.ones_like(
                    segmentation_list[glob_ind])
                end_img = self.back_id_list[glob_ind + 1] * np.ones_like(
                    segmentation_list[glob_ind + 1])
                disapp_img = self.back_id_list[glob_ind] * np.ones_like(
                    segmentation_list[glob_ind])
                app_img = self.back_id_list[glob_ind + 1] * np.ones_like(
                    segmentation_list[glob_ind + 1])
                for ind, node in enumerate(
                        glob_val):  # ind ne sert a rien, nds on itere sur les nodes de ce temps la
                    if node in matching_dict:
                        if matching_dict[node]['Event'] in ['Stay', 'Split']:
                            ...
                            if matching_dict[node]['Event'] == 'Stay':
                                lab_set_2 = matching_dict[node][
                                    'Initial label set 2']
                                init_img[segmentation_list[
                                             glob_ind] == lab_set_1] = lab_set_1
                                end_img[segmentation_list[
                                            glob_ind + 1] == lab_set_2] = lab_set_1
                            elif matching_dict[node]['Event'] == 'Split':
                                lab_1_set_2 = matching_dict[node][
                                    'Initial label 1 set 2']
                                lab_2_set_2 = matching_dict[node][
                                    'Initial label 2 set 2']
                                init_img[segmentation_list[
                                             glob_ind] == lab_set_1] = lab_set_1
                                end_img[segmentation_list[
                                            glob_ind + 1] == lab_1_set_2] = lab_set_1
                                end_img[segmentation_list[
                                            glob_ind + 1] == lab_2_set_2] = lab_set_1
                                val = lab_1_set_2 in np.unique(
                                    segmentation_list[glob_ind + 1]).tolist()

                        elif matching_dict[node][
                            'Event'] == 'Disappear':  # ,'Appear']:
                            if matching_dict[node]['Event'] == 'Disappear':
                                lab_set_1 = matching_dict[node]['Label set 1']
                                disapp_img[segmentation_list[
                                               glob_ind] == lab_set_1] = lab_set_1

                    for tmp in matching_dict:
                        if matching_dict[tmp]['Event'] == 'Appear':
                            lab_set_2 = matching_dict[tmp]['Label set 2']
                            app_img[segmentation_list[
                                        glob_ind + 1] == lab_set_2] = lab_set_2

                init_sp_img = SpatialImage(init_img,
                                           voxelsize=segmentation_list[
                                               glob_ind].voxelsize)
                end_sp_img = SpatialImage(end_img, voxelsize=segmentation_list[
                    glob_ind + 1].voxelsize)
                disapp_sp_img = SpatialImage(disapp_img,
                                             voxelsize=segmentation_list[
                                                 glob_ind].voxelsize)
                app_sp_img = SpatialImage(app_img, voxelsize=segmentation_list[
                    glob_ind + 1].voxelsize)
    ...
    def write_lineage_file(self, out_path, matching_dict, times=None):
        ...
        filename = out_path + 'a_lineage_time_' + str(times[0]) + '_to_' + str(
            times[1]) + '.txt'
        outfile = open(filename, 'a')
        ent = 'Initial time: ' + str(times[0]) + ' - Final time: ' + str(
            times[1]) + '\n'
        outfile.write(ent)
        outfile.write('STAY \n')
        for key in matching_dict:
            if (matching_dict[key]['Event'] == 'Stay'):
                ...
                match_str = ''.join(
                    ['[', str(label_set_1), '] [', str(label_set_2), '] [',
